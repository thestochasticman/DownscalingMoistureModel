...
> **Provenance.** This nine-date gallery is generated by
> [`plot_downscale_gallery.py`](../plot_downscale_gallery.py) with **model4**,
> the ML track's production model at the time it was made; it shows what
> downscaling *does*, not the current best skill. The equivalent pair from the
> recommended ensemble is on the
> [nn-stack page](nn_stack.md#what-it-looks-like-on-the-ground).
