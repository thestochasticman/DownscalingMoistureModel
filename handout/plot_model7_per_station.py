"""model7 held-out per-station series, from the cached LOSO predictions.

Reads data/model7t_loso_predictions.csv (the terrain-offset variant — the
reported model7 configuration), written by the evaluation run; nothing
re-calibrates here.

Run from repo root::  PYTHONPATH=. python handout/plot_model7_per_station.py
"""
...
FIG = REPO / "handout" / "figures" / "model7_per_station.png"
...
pred = pd.read_csv(REPO / "data" / "model7t_loso_predictions.csv", parse_dates=["time"])
...
fig.suptitle(f"model7 (bucket + terrain offsets) leave-site-out prediction vs observation, "
             f"all {len(stations)} stations (2006-2010).  Pooled NSE = {pooled['nse']:+.2f}, "
