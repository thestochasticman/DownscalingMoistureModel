...
def station_features(station: str, site: str, lat: float, lon: float,
                     ...
                     buffer_km: float = 1.5, smips_var: str = "totalbucket") -> pd.DataFrame:
    ...
    # SMIPS time series at the station pixel.
    cube = download_smips(q, var=smips_var)
    smips_ts = sample_points(cube, lon, lat).to_pandas()
    smips_ts.index = pd.to_datetime(smips_ts.index).normalize()
    df[SMIPS_COL] = df["time"].map(smips_ts)
...
def build_training_table(coords: pd.DataFrame, oznet_daily: pd.DataFrame,
    ...
    """
    frames = []
    rows = coords.dropna(subset=["lat", "lon"])
    for i, r in enumerate(rows.itertuples(index=False), 1):
        ...
        try:
            f = station_features(r.station, site, float(r.lat), float(r.lon),
                                 sub, start, end, buffer_km=buffer_km, smips_var=smips_var)
        except Exception as e:
            if verbose:
                print(f"  [{i}/{len(rows)}] {r.station}: SKIP ({type(e).__name__}: {e})")
            continue
        if not f.empty:
            frames.append(f)
        if verbose:
            print(f"  [{i}/{len(rows)}] {r.station}: {len(f)} rows", flush=True)
